...
class LiftEnv(gym.Env):
    ...
    def _save_demo_frames(self, step_i, action):
        ...
        img_save_dir = os.path.join("lift_rl_data", self.save_path, "demo")
    ...
    def _log_episode_stats(self, stats, action):
        ...
        # Always log stats
        with open(f"lift_logs/{self.save_path}/episode_stats.txt", "a") as file:
            file.write(str(stats) + "\n")

        # If collecting data, also log success/action
        if self.collect_data:
            with open(f"lift_rl_data/{self.save_path}/success_rate.txt", "a") as file:
                file.write(str(stats["Success_Rate"]) + "\n")
            with open(f"lift_rl_data/{self.save_path}/actions.txt", "a") as file:
