...
class CanEnv(gym.Env):
    ...
    def reset(self):
        ...
        # Example: restore some default colors or remove them
        for geom in root.findall(".//geom"):
            if "robot" in geom.attrib.get("name", ""):
                if "rgba" in geom.attrib:
                    del geom.attrib["rgba"]

        for light in root.findall(".//light"):
            # Example: reset lighting to some default
    ...
    def _save_demo_frames(self, step_i, action):
        ...
        img_save_dir = os.path.join("can_rl_data", self.save_path, "demo")
    ...
    def _log_episode_stats(self, stats, action):
        ...
        # Always log stats
        with open(f"can_logs/{self.save_path}/episode_stats.txt", "a") as file:
            file.write(str(stats) + "\n")

        # If collecting data, also log success/action
        if self.collect_data:
            with open(f"can_rl_data/{self.save_path}/success_rate.txt", "a") as file:
                file.write(str(stats["Success_Rate"]) + "\n")
            with open(f"can_rl_data/{self.save_path}/actions.txt", "a") as file:
