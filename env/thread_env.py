...
class ThreadEnv(gym.Env):
    ...
    def reset(self):
        ...
        #cube color
        for geom in root.findall(".//geom"):
            if geom.attrib['name'] == 'needle_obj_handle_vis':
                if 'rgba' in geom.attrib:
                    del geom.attrib['rgba'] 

            if "name" in geom.attrib and geom.attrib["name"] == "gripper0_finger1_visual":
                    geom.set("rgba", "0.499 0.499 0.499 1")
            if "name" in geom.attrib and geom.attrib["name"] == "gripper0_finger2_visual":
                    geom.set("rgba", "0.499 0.499 0.499 1") 
    ...
    def step(self, action):
        ...
        for geom in root.findall(".//geom"): 
            # cylinder size
            if action == 0:
                if "name" in geom.attrib and geom.attrib["name"] == "needle_obj_handle_vis":
                    geom.attrib["size"] = "0.025 0.02 0.02" 
            elif action == 1:
                if "name" in geom.attrib and geom.attrib["name"] == "needle_obj_handle_vis": # Cube green color
                    geom.attrib['size'] = "0.025 0.02 0.03"
            elif action == 2:
                if "name" in geom.attrib and geom.attrib["name"] == "needle_obj_handle_vis": # Cube blue color
                    geom.attrib['size'] = "0.02 0.025 0.02"
            elif action == 3:
                if "name" in geom.attrib and geom.attrib["name"] == "needle_obj_handle_vis": 
                    geom.attrib['size'] = "0.02 0.02 0.025"
    ...
    def _save_demo_frames(self, step_i, action):
        ...
        img_save_dir = os.path.join("thread_rl_data", self.save_path, "demo")
    ...
    def _log_episode_stats(self, stats, action):
        ...
        # Always log stats
        with open(f"thread_logs/{self.save_path}/episode_stats.txt", "a") as file:
            file.write(str(stats) + "\n")

        # If collecting data, also log success/action
        if self.collect_data:
            with open(f"thread_rl_data/{self.save_path}/success_rate.txt", "a") as file:
                file.write(str(stats["Success_Rate"]) + "\n")
            with open(f"thread_rl_data/{self.save_path}/actions.txt", "a") as file:
